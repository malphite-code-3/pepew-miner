...
#if defined(_M_X64) || defined(__x86_64__)#include <wmmintrin.h>
...
static inline uint64_t Divide128Div64To64(uint64_t high, uint64_t low, uint64_t divisor, uint64_t * remainder) {
  uint64_t result;
  __asm__("divq %[v]": "=a"(result), "=d"( * remainder) // Output parametrs, =a for rax, =d for rdx, [v] is an
    // alias for divisor, input paramters "a" and "d" for low and high.
    : [v]
    "r"(divisor), "a"(low), "d"(high));
  return result;
}
...
void aes_single_round_no_intrinsics(uint8_t * state,
  const uint8_t * round_key) {
  sub_bytes(state);
  shift_rows(state);
  mix_columns(state);
...
void static inline aes_single_round(uint8_t * block,
  const uint8_t * key) {
